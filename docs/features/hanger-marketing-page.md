# Feature: "Hanger" — GitHub Pages Marketing Site

> **Audience:** maintainers and AI agents implementing the public marketing page for this fork
> (`thirschel/claude-squad`). This is a **feature specification**, not an implementation. It defines
...
> Appendix B. **Scope:** marketing-page branding only — the repo, Go module (`claude-squad`), and
> `cs` binary are **not** renamed in this feature (see Appendix A).
...
**"Hanger"** landing page deployed to GitHub Pages. The page showcases the app's features —
**led by this fork's native-Windows support** — and gives basic start/install instructions.
"Hanger" is the page brand with a working tagline like *"a hanger for all your copilots."*
...
**Goals**
- A single-page marketing site branded **"Hanger"** with a memorable tagline.
...
- Point links/metadata at this fork (`thirschel/claude-squad`) where appropriate, preserving the
...
- Renaming the GitHub repo, Go module, or `cs` binary (tracked separately — Appendix A).
...
| Aspect | Decision |
|--------|----------|
| **Name** | **Hanger** (page brand only; product/binary stay `claude-squad` / `cs`). |
| **Tagline (working)** | *"A hanger for all your copilots."* |
| **Tagline alternatives** | "Hang all your copilots in one place." · "Where your copilots hang." · "One rack for every AI copilot." · "Rack up your AI agents." |
...
> **Open branding question (flag for review):** "Hanger" (coat hanger) vs "Hangar" (where aircraft —
> i.e., *pilots/copilots* — are housed). The aviation spelling arguably fits the "copilot" metaphor
> better. We keep **"Hanger"** per the maintainer's request but list this as an explicit, low-cost
> decision to revisit before launch.
...
1. **Header** — "Hanger" wordmark; links: GitHub (this fork), Docs (README), theme toggle.
...
**Branding & content**
- [ ] The page `<h1>`, browser tab title, and `metadata` all read **"Hanger"** (no visible
...
**Links & metadata**
- [ ] All GitHub/repo/raw links point at **`thirschel/claude-squad`** (not `smtg-ai`), **except**
...
- [ ] `layout.tsx` `metadata` (title, description, keywords) reflects "Hanger".
- [ ] OpenGraph/Twitter **canonical URL** uses the published site URL
      **`https://thirschel.github.io/claude-squad/`**; repository links use
      `https://github.com/thirschel/claude-squad`. `metadataBase` is set to the site URL so relative
      OG image paths resolve absolutely (avoids the Next `metadataBase` build warning).
- [ ] A Hanger-branded **OpenGraph/Twitter image** exists (e.g. `web/public/og-hanger.png`,
...
- [ ] The `Deploy to GitHub Pages` workflow succeeds on PR (build job) and on push to `main`
      (build + deploy jobs).
- [ ] `next.config.ts` `basePath` stays **`/claude-squad`** while the repo keeps that name (see §9
      and Appendix A). The built `web/out` is verified **served under `/claude-squad/`**: every asset
      and internal link resolves at that base path (no 404s for `/claude-squad/_next/...`).
...
- [ ] The export produces a `404.html`; it is either Hanger-branded (`not-found.tsx`) or an
...
3. **`basePath`-aware link check** — serve the built `out/` at
   `http://localhost:PORT/claude-squad/` and run `linkinator`/`lychee`, asserting 2xx/3xx for
   internal links **and assets** (`/claude-squad/_next/...`) and for in-page **hash anchors**
   (`#features`, `#install`). Serving `out/` at the server root instead produces false 404s.
4. **Brand/leak check** — grep built `out/**/*.html`: assert **"Hanger"** appears in `<title>` and
...
5. **OG/meta assertion** — parse `out/index.html`: `og:image`/`twitter:image` resolve to a 200 asset
   (under the base path) and the card is `summary_large_image`; `title`/`description` are the Hanger
   values.
...
7. **Smoke test (Playwright)** — load the exported page **served under `/claude-squad/`** and assert:
   `<h1>` = "Hanger"; a native-Windows feature heading is visible; install code blocks present; theme
...
| File | Change |
|------|--------|
| `web/src/app/page.tsx` | Replace headline/tagline; rewrite feature list into the six items (§4); reorder install blocks to Windows-first (§5); fix prerequisites; repoint links to `thirschel/claude-squad` **except** the upstream Unix install commands and footer attribution (§7). |
| `web/src/app/layout.tsx` | Update `metadata` title/description/keywords to "Hanger"; set OpenGraph/Twitter to the **site** URL `https://thirschel.github.io/claude-squad/`; set `metadataBase` to that URL; add `openGraph.images`/`twitter.images` pointing at the OG image. |
...
| `web/public/` | Add the Hanger OG image (e.g. `og-hanger.png`, 1200×630) and any wordmark. **Asset landmine:** assets referenced via plain `<img src="/…">` must include the `basePath` (`/claude-squad/og-hanger.png`) or they 404 on Pages. |
| `web/next.config.ts` | **Keep** `basePath: "/claude-squad"` for production (repo name unchanged → becomes `/hanger` only on a future rename, Appendix A). Add `images: { unoptimized: true }` **iff** any `next/image` is introduced (required for `output: "export"`). |
...
- Published URL will be **`https://thirschel.github.io/claude-squad/`** (project-pages path matches
...
- **basePath vs brand:** URL path stays `/claude-squad` while the brand is "Hanger" — acceptable for
  a branding-only pass, but is the path/brand mismatch okay for launch? (Yes, per scope; revisit with
  Appendix A.)
- **"Hanger" vs "Hangar":** spelling/metaphor decision (see §3).
...
## Appendix A — Future work: full "Hanger" rename (out of scope here)

If the project later adopts "Hanger" as its real name, a separate effort would cover: GitHub repo
rename (and its Pages URL → `basePath` change to `/hanger`), Go module path `claude-squad`,
the `cs` binary name and install scripts (`install.sh`, `install.ps1`, `install.bat`, Homebrew
formula), all in-code/string references, and CI/release config (`.goreleaser.yaml`). That work is
**explicitly excluded** from this feature and noted here only so the marketing page can be designed
to make a future rename low-friction.
...
| 2 | `basePath` landmine — serving `out/` at root, or `/`-rooted assets, 404s on Pages. | Gemini, Opus | Added AC + test step requiring `out/` served under `/claude-squad/`; added `public/` asset-prefix note in §9. |
