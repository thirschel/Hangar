...
type TmuxSession struct {
	...
	// lookPath resolves a program name to an executable path (exec.LookPath by
	// default; overridable in tests). Used to diagnose why a session died.
...
const TmuxPrefix = "hangar_"

var whiteSpaceRegex = regexp.MustCompile(`\s+`)

func toHangarTmuxName(str string) string {
...
func newTmuxSession(name string, program string, ptyFactory PtyFactory, cmdExec cmd.Executor) *TmuxSession {
	return &TmuxSession{
		sanitizedName: toHangarTmuxName(name),
		program:       program,
		ptyFactory:    ptyFactory,
		cmdExec:       cmdExec,
...
func (t *TmuxSession) Start(workDir string) error {
	...
	// A program that exits the instant it launches (missing binary, wrong
	// architecture, incompatible glibc, a Windows shim under /mnt, etc.) leaves
	// a dead session whose pane we'd later fail to capture with a cryptic
	// "can't find pane". Detect that here and surface the real reason instead.
	if !t.DoesSessionExist() {
		startErr := t.diagnoseStartupFailure()
		if cleanupErr := t.Close(); cleanupErr != nil {
...
// diagnoseStartupFailure builds an actionable error explaining why a freshly
// started session died immediately.
func (t *TmuxSession) diagnoseStartupFailure() error {
	return diagnoseProgram(t.program, t.lookPath, runtime.GOOS, t.probeProgram, log.LogFileName())
}

// diagnoseProgram inspects why the configured program failed to stay running.
// It is split out (with injected dependencies) so it can be unit tested.
func diagnoseProgram(program string, lookPath func(string) (string, error), goos string, probe func() string, logPath string) error {
	program = strings.TrimSpace(program)
	fields := strings.Fields(program)
	if len(fields) == 0 {
		return fmt.Errorf("no program is configured to run in the session")
	}
	bin := fields[0]
	base := fmt.Sprintf("program %q exited immediately after the tmux session started", program)

	resolved, lookErr := lookPath(bin)
	if lookErr != nil {
		return fmt.Errorf("%s: %q was not found on PATH — install it in this environment "+
			"(on WSL, install the agent inside Linux, not Windows)", base, bin)
	}
	if goos == "linux" && strings.HasPrefix(resolved, "/mnt/") {
		return fmt.Errorf("%s: %q resolves to a Windows executable (%s) that cannot run inside a "+
			"Linux tmux session — install the agent natively in WSL/Linux", base, bin, resolved)
	}
	if probe != nil {
		if reason := probe(); reason != "" {
			return fmt.Errorf("%s: %s", base, reason)
		}
	}
	hint := "verify it runs standalone in this environment"
	if logPath != "" {
		hint = fmt.Sprintf("%s (see logs at %s)", hint, logPath)
	}
	return fmt.Errorf("%s — %s", base, hint)
}

// probeProgram runs the configured program directly with a short timeout and
// returns the last line of any output it produced while starting. A program
// that launches a normal interactive UI keeps running until the timeout
// (returning nothing); one that crashes on launch returns its error quickly
// (e.g. a "GLIBC_2.28 not found" loader error).
func (t *TmuxSession) probeProgram() string {
	...
	defer cancel()
	probe := exec.CommandContext(ctx, fields[0], fields[1:]...)
	var out bytes.Buffer
	probe.Stdout = &out
	probe.Stderr = &out
	_ = probe.Run()
	if ctx.Err() == context.DeadlineExceeded {
		return "" // still running at timeout => it launches fine
	}
...
func (t *TmuxSession) CapturePaneContent() (string, error) {
	...
	if err != nil {
		return "", annotateCaptureError("error capturing pane content", err)
	}
...
func (t *TmuxSession) CapturePaneContentWithOptions(start, end string) (string, error) {
	...
	output, err := t.cmdExec.Output(cmd)
	if err != nil {
		return "", annotateCaptureError("failed to capture tmux pane content with options", err)
	}
	return string(output), nil
}

// annotateCaptureError enriches a capture-pane failure. Thanks to the command
// executor surfacing stderr, err typically already contains tmux's real message
// (e.g. "can't find pane"); when that indicates the pane/session is gone we add
// a hint that the agent process exited.
func annotateCaptureError(action string, err error) error {
	msg := err.Error()
	if strings.Contains(msg, "can't find pane") || strings.Contains(msg, "can't find session") {
		return fmt.Errorf("%s: %w — the agent process appears to have exited; verify it runs "+
			"standalone in this environment (logs: %s)", action, err, log.LogFileName())
	}
